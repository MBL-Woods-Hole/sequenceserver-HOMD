...
$jb_tracks = "DNA,prokka,prokka_ncrna,ncbi,ncbi_ncrna,GC Content (pivot at %d),GC Skew"
...
def get_stats (dbtypex, idx, titlex)
    ...
    if dbtypex == 'protein'
       if idx.include? "|"
        ...
        elsif idx.include? "|"   # No sql needed
...
=end
        ...
        if stats[:seqtype] != 'fna'  #
            if stats[:seqtype] == 'ffn' && stats[:anno] == 'NCBI' && stats[:pid] == nil && stats[:acc]
              ...
              url += "&tracks="+ ERB::Util.url_encode($jb_tracks % gc.to_f)
            else
                ...
                if rs.count > 0
                    ...
                    url += "&loc=#{seq_id}|#{acc}:#{locfrom}..#{locto}"
                    url += "&highlight=#{seq_id}|#{acc}:#{hitfrom}..#{hitto}"
